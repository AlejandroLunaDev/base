...
Vercel detecta cambios en la rama conectada (`staging` o `main`) y hace auto-deploy automáticamente, incluso si los cambios son solo en `backend/` y no afectan al frontend.
...
2. **Usa GitHub Actions para deployar** (workflows `staging-deploy.yml` y `production-deploy.yml`)
